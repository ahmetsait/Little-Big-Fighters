 ...
 * 10 bit significand.
 * All operations on Half are CTFE'able.
 *
 * References:
 ...
 * $(UL
 * $(LI explicit conversion of float to Half)
 * $(LI implicit conversion of Half to float)
 * )
...
struct Half
{
	/* Provide implicit conversion of Half to float
     */
	...
	///
	static @property Half min_normal() { Half hf = void; hf.s = 0x0400; return hf; }
	unittest { assert(min_normal == hf!0x1p-14); }
	
	///
	static @property Half max()        { Half hf = void; hf.s = 0x7BFF; return hf; }
	unittest { assert(max == hf!0x1.FFCp+15); }
	
	///
	static @property Half nan()        { Half hf = void; hf.s = EXPMASK | 1; return hf; }
	//unittest { assert(nan != hf!(float.nan)); }
	
	///
	static @property Half infinity()   { Half hf = void; hf.s = EXPMASK; return hf; }
	unittest { assert(infinity == hf!(float.infinity)); }
	
	///
	static @property Half epsilon()    { Half hf = void; hf.s = 0x1400; return hf; }
...
private:
	ushort s = EXPMASK | 1;     //.init is Half.nan
}
...
unittest
{
	...
	assert(f == Half.epsilon);
	
	f.s = 0x0400;
	//writeln("6.10352e-5 ", cast(float)f);
	assert(f == Half.min_normal);
